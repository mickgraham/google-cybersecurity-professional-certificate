...
Common intrusion network attacks include:

* Malware
* Spoofing
* Packet sniffing

Attacks can harm an organisation by:
...
* **Denial of Service (DoS):** A network attack that targets a network or server and floods it with network traffic.
* **Distributed Denial of Service (DDoS):** A DoS attack that uses multiple devices or servers in different locations to flood the target network with unwanted traffic.
* **SYN Flood:** A type of DoS attack that simulates a TCP/IP connection and floods a server with SYN packets.
* **Packet Sniffing:** The practice of capturing and inspecting data packets across a network.
* **IP Spoofing:** A network attack performed when an attacker changes the source IP of a data packet to impersonate an authorised system and gain access to a network.
* **On-Path:** An attack where a malicious actor places themselves in the middle of an authorised connection and intercepts or alters the data in transit.
* **Smurf:** A network attack where an attacker sniffs an authorized user's IP address and floods it with packets.
